...
// Layer 2: Built-in knowledge base (keyword matching, offline)
// Layer 3: Help Center (triggered separately — always offline)
// Layer 4: Contextual field tooltips (per-input, always offline)
//
// The rest of the app is completely unaffected if the API is unavailable.
// No feature outside this hook is gated behind API availability.
...
const AUTO_RETRY_MS    = 30000;   // 30 seconds
const MAX_HISTORY      = 20;      // message pairs kept in context window


// ── Knowledge base ────────────────────────────────────────────────────────────
// Used when the API is unavailable and the user's message matches a topic.
 ...
 * @param {Array}        params.semesterSummaries
 * @param {Object}       params.totals           — { totalCU, totalQP }
 * @param {number}       params.activeScale
 * @param {Array}        params.activeClassifications
 */
export function useChat({
  ...
  async function callAPI(userMessage) {
    ...
    // Build conversation history for context (most recent MAX_HISTORY messages)
  ...
  const sendMessage = useCallback(
    async (userMessage) => {
      ...
      try {
        ...
        // Abort is not an error — user navigated away
        if (err.name === "AbortError") {
          setIsLoading(false);
          return;
        }

        // Mark API as offline for subsequent messages
        setIsAPIOnline(false);
      }

      // ── Fallback Layer 2: Knowledge base ──────────────────────────────────
      ...
      // ── Fallback Layer 1: Unavailability notice ───────────────────────────
  ...
  const retryMessage = useCallback(
    async (payload) => {
      ...
      // Replace the error message with a loading state
      setMessages((prev) =>
        prev.map((m) =>
